-- Drop the views that have security definer issues and recreate them without SECURITY DEFINER
DROP VIEW IF EXISTS public.users_with_profiles;
DROP VIEW IF EXISTS public.admin_statistics;

-- Create users_with_profiles view without SECURITY DEFINER
...
-- Create admin_statistics view without SECURITY DEFINER
...
-- Enable RLS on views
ALTER TABLE public.users_with_profiles ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.admin_statistics ENABLE ROW LEVEL SECURITY;

-- RLS policies for users_with_profiles
CREATE POLICY "Users can view their own profile"
  ON public.users_with_profiles
  FOR SELECT
...
-- RLS policies for admin_statistics
CREATE POLICY "Only superadmins can view statistics"
